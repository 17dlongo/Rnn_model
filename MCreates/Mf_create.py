...
def create_XY(stocks,start,end,length,exPerBatch,Features):
 ...
 #       print((stocks,date__str(startb),date__str(endb)))
    ...
    final = []
    '''
    for i in range(len(batches_X)): #for each batch
        new = []
        for y in range(len(batches_X[i])): # for each example per batch
            neww = []
            for z in range(len(batches_X[i][y])): # for each vector per batch
                add = tf.boolean_mask(batches_X[i][y][z],batchesM_X[i][y][z])
                if len(add.eval()) == 0:
                #    print("None")
                    continue

                neww = stack(neww,(add.eval()))
            new = stack(new,neww)
        final = stack(final,new)
    '''
    for i in range(len(batches_X)): #for each batch
        new = []
        for y in range(len(batches_X[i])): # for each example per batch
            ...
            new = stack(new,neww)
        final = stack(final,new)
    print(np.squeeze(np.array(final)).shape)
    print(np.array(batches_X).shape)
    print(np.squeeze(np.array(final)))
    print(np.array(batches_X))
'''
    for item in batches_X: #batches
        for item in batches_X
    for i in range(len(batches_X)):
        for y in range(length+1):
            print(batches_X[i])
            print("ITTTT",batches_X[i][y])
            add = tf.boolean_mask(batches_X[i][y],batchesM_X[i][y]).eval()
        #    print(add)
'''
    #return (batches_X,batches_Y,batchesM_X,batchesM_Y)
